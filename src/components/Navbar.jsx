...
function Navbar({ theme, setTheme }) {
    ...
    return (
        <motion.div
            ...
            <img src={theme == 'dark' ? assets.logo_dark : assets.logo} alt="Logo" className="w-32 sm:w-40 cursor-pointer" />

            <div className={`text-gray-700 dark:text-white sm:text-sm ${sidebarOpen ? 'max-sm:w-60 max-sm:pl-10' : 'max-sm:w-0 overflow-hidden'} 
                max-sm:fixed top-0 right-0 bottom-0 max-sm:min-h-screen max-sm:h-full max-sm:flex-col max-sm:bg-primary max-sm:text-white 
                max-sm:pt-20 flex sm:items-center gap-5 transition-all`}
            >
                <img src={assets.close_icon} alt='Menu bar close icon' className='w-5 absolute right-4 top-4 sm:hidden'
            ...
            <div className='flex items-center gap-2 sm:gap-4'>
                ...
                <img src={theme == 'dark' ? assets.menu_icon_dark : assets.menu_icon} alt='Menu bar open icon'
